...
graph TD
    ...
    B1 -- "a" --> B2 -- "n" --> B3 -- "a" --> B4 -- "n" --> B5 -- "a" --> L1["Leaf (0)"]
    A1 -- "n" --> A1_2 -- "a" --> A1_3 -- "n" --> A1_4 -- "a" --> L2["Leaf (1)"]
    N1 -- "a" --> N1_2 -- "n" --> N1_3 -- "a" --> L3["Leaf (2)"]
    A1 -- "n" --> A2_2 -- "a" --> L4["Leaf (3)"]
    N1 -- "a" --> L5["Leaf (4)"]
    A1 -- "空" --> L6["Leaf (5)"]

    %% エッジラベルの圧縮を表現
    %% 各ノードのラベルは、banana$ の各接尾辞の開始インデックス

    %% 補足: 実際のSuffix Treeではエッジラベルは元文字列のインデックス範囲で管理される
```
