...
- **tidwall/rtree**: 高性能な地理空間データベースのプロジェクト Tile38 向けに設計されたライブラリ。高速な分割アルゴリズムを採用し、Go のジェネリクスを取り入れて実装されている。ただし構造の最適性は劣る可能性がある。挿入スループットが重要な動的な高負荷システムに有利。
...
- **kd-tree**: k 次元空間内の点データを整理するための空間分割データ構造であり、指定された矩形領域（クエリ領域）に含まれるすべての点を見つける範囲探索や、与えられたクエリ点に最も近い点を見つける最近傍探索に特化している
