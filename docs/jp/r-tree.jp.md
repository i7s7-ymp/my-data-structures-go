...
R-tree とよく比較対象になるのが、kd-tree である。これらは異なる問題領域を対象としている。
...
| ------------------ | ----------------------------------------------------- | ---------------------------------------------------------- | ------ |
| **検索対象データ** | 空間オブジェクト（点、線、矩形、多角形）を MBR で表現 | k 次元の点データ                                           |        |
| **木構造**         | 高さ平衡木（B-tree に類似）                           | 二分木（動的なデータに対しては本質的に不平衡になりやすい） |        |

| **主な用途** | 空間オブジェクトの窓クエリ・交差クエリ (GIS) | 点データの最近傍探索 (機械学習) | |
| **ノード構造** | 可変数の子ノード（高いファンアウト） | 常に 2 つの子ノード（二分木） | |

| **空間検索** | ✅ 最適 | ⚠️ 点データ向け | ❌ 不適 |
| **範囲検索** | ✅ O(log n + k) | ⚠️ O(√n + k) | ❌ O(n) |
| **挿入・削除** | ✅ O(log n) | ❌ O(n) | ✅ O(log n) |
| **動的更新** | ✅ 効率的 | ❌ 再構築必要 | ✅ 効率的 |
| **メモリ効率** | ✅ 良好 | ✅ 良好 | ✅ 最高 |
| **矩形データ** | ✅ 最適 | ❌ 不適 | ❌ 不適 |
| **実装複雑度** | ❌ 複雑 | ✅ 比較的簡単 | ⚠️ 中程度 |
