...
Set の実装の選択は、以下の 2 点を考慮して決定する。

- データが持つ構造的特性（順序性）
- 実行される操作の種類（ピンポイントのキーの検索か、範囲検索か）

代表的な HashSet と TreeSet について比較してみる。この二つの性能の違いは「速度と順序性のトレードオフ」として表現することができる。

- **HashSet**: 要素の存在確認や一意性の確認を平均 O(1) の計算量で実行できる
- **TreeSet**: 存在確認・一意性の確認には O(log n)かかるが、要素の完全な順序性を保証できる
  - この順序性により以下のクエリを効率的に実行できる。HashSet で実現するには O(n) かかる。
    - 特定のキーより大きい最初の要素を探す
    - ある範囲内の全ての要素を列挙する

以上から、この二つの実装を選択する上では、速度ではなく、O(log n)のオーバーヘッドを払ってでも、順序性を維持したり範囲クエリを可能にすることに価値を置くか、検討するのが重要である。

他の実装方法も併せた比較の表は以下の通り。
...
Set が示す重複排除や存在確認の特性を利用した活用例は数多くある。

- **データの重複排除**: 大量のデータストリームやリストから重複する要素を効率的に除去する
- **存在確認とキャッシュ**: ウェブクローラーが既に訪れた URL を記録し、再訪を防ぐケースや、キャッシュシステムで特定のキーが存在するかを高速に判定する際に利用される
