...
Suffix-tree（接尾辞木）は、ある文字列のすべての接尾辞（suffix）を格納した、圧縮されたトライ木である。文字列の各位置から始まるすべての部分文字列を圧縮された形で格納し、文字列の様々な操作を高速に実行できる。テキストを一度前処理しておくことで、その後の複雑な部分文字列クエリに極めて高速に応答することを可能にする。

## 文字列 "banana$" の Suffix-tree 例

「banana$」という文字列を、Suffix Tree で管理してみる。なお、末尾の「$」は終端文字と呼ばれ、全ての接尾辞が必ず葉ノードで終わるようにするための文字列である。通常は文字列に現れない特殊文字を利用する。

「banana$」から作成できる設備時は以下の 6 種類である。
...
先頭にくる文字は「b」「a」「n」のいずれかなので、ルートノードから 3 種類のエッジを伸ばし「内部ノード」に接続する。「内部ノード」は、複数の接尾辞が分岐する点である。また、ルートノードから分岐する時点で終端文字は考慮しない。
...
次に、「banana$」が作れる接尾辞より、「b」「a」「n」から派生できる接尾辞をそれぞれの「内部ノード」から派生して作成する。

ある内部ノードから葉ノードまでパスが一位で定められる場合、パスを圧縮することができる（エッジラベルの圧縮）。実際の Suffix Tree ではエッジラベルは元文字列のインデックス範囲で管理される

````
各ノードのラベルは「banana$」の各接尾辞の開始 index を表す。
...
ルートノードから各葉ノードへのパスは、文字列の一意な接頭辞に対応する。ノードの種類は主に 3 つ。内部ノード（2 つ以上の子を持つノード）は、文字列 T 内で 2 回以上出現する反復部分文字列に対応する。特に、根から内部ノードへのパス文字列が、その反復部分文字列となる。

- **ルートノード（Root Node）**: 木構造の最上位ノード
- **内部ノード（Internal Node）**: 複数の接尾辞が分岐する点
- **リーフノード（Leaf Node）**: 接尾辞の終端を表すノード

全ての接尾辞が必ず葉ノードで終わるようにするため、元の文字列の末尾に、その文字列に現れない特殊な文字（終端文字、$ など）を付加する。
...
上記の特徴により、文字列 T（長さ n）に対する部分文字列 P（長さ m）の存在確認は、元の文字列の長さに依存しない時間計算量 O(m)で実行できる。これは、Suffix Tree をたどることで、P が T のいずれかの接尾辞の接頭辞であるかどうかを判定できるためである。

## ユースケース

O(n) の前処理の後、任意のパターン(長さ $m$)がテキスト内に存在するかを O(m) で検索できる。その他、最長共通部分文字列の発見、最長反復部分文字列の発見、DNA シーケンス解析などのバイオインフォマティクス分野で利用される。
...
## index/suffixarray による Suffix Array を用いた実装

Go は標準ライブラリ index/suffixarray によって、部分文字列検索の基本的な操作を行うことができる。

以下は「banana$」という文字列から「ana」という文字列を検索する例を示す。
...
func main() {
    ...
    // "ana"が出現する全ての位置を検索
    // n=-1 は全ての一致を返すことを意味する
    offsets := indetx.Lookup(byte("ana"), -1)
