...
| 実装方法   | メリット                                                                                   | デメリット                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 配列       | - メモリが連続しておりキャッシュ効率が高い<br>- 両端の操作が効率的（固定サイズ内での操作） | - サイズ変更不可、事前にサイズを決定する必要がある<br>- サイズ超過時にオーバーフローが発生     |
| スライス   | - 動的にサイズ変更可能<br>- 標準ライブラリを活用でき実装が簡単                             | - 両端操作でスライスの再割り当てが発生し、パフォーマンス低下<br>- 頻繁な再割り当てで効率が悪化 |
| 連結リスト | - 両端の操作が効率的（O(1)）<br>- サイズが動的に変化しメモリ効率が良い                     | - メモリが非連続でキャッシュ効率が低い<br>- 各ノードにポインタを持つためメモリ使用量が増加     |

## 配列を使用したデックの実装

- **固定サイズ**: 配列を使用するため、デックのサイズは固定。
- **メモリ効率**: 必要な分だけメモリを使用するため、メモリの無駄が少ない。
- **インデックス管理**: 両端の操作を効率的に行うため、head と tail をインデックスで管理。
- **循環構造**: 配列の末尾が先頭に接続されるように設計することで、メモリを効率的に利用可能。
...
- **スライスを使用**: デックのデータを保持するためにスライスを使用。
- **動的サイズ**: スライスの動的サイズ変更機能を活用し、柔軟にサイズを変更可能。
- **両端操作**: PushFront ではスライスの先頭に要素を追加（append([]int{value}, slice...)）、PushBack では末尾に追加（append(slice, value)）。
- **計算量**: 両端操作でスライスの再割り当てが発生する場合、計算量が O(n) になる可能性がある。
...
- **動的サイズ**: 連結リストを使用することで、デックのサイズを動的に変更可能。
- **両端操作**: 両端の操作（PushFront、PushBack、PopFront、PopBack）が O(1) で実行可能。
- **メモリ効率**: 必要な分だけメモリを使用するため、メモリの無駄が少ない。
