...
- ハッシュテーブル配列の各要素
- 実際にデータが格納され、これ自体が配列になっている
- 各要素（バケット）にキー・バリューペアを格納する

## 特徴

- **キー・バリュー型**: キーと値のペアでデータを管理
- **ハッシュ関数**: キーを配列のインデックスに変換する関数
- **バケット配列**: ハッシュテーブル全体の配列の要素であり、キー・値のペアを格納する配列
...
bmap というバケット構造体の配列が元になっている。

bmap とは、最大で 8 個のキーと値のペアを格納できるスロットを持つバケット。

メモリの配列とパディングの削減などのパフォーマンス最適化のため、バケット内では 8 個のキーが連続した領域に、その後ろの 8 この値が連続して配置される構造になっている。

### map 実装時のキー検索のプロセス

1. **ハッシュ値の生成**: ハッシュ値の計算には、キーと、map ごとにランダム生成される「ハッシュシード」の二つが用いられる。シードをランダムにすると、ハッシュ衝突を狙った攻撃（ハッシュフラッディング攻撃）を緩和する。
2. **バケットの選択**: 生成されたハッシュ値の下位ビットをバケット配列の index として使用。どのバケットを最初に探索するかは、この下位ビット index の値で決まる。
3. **tophash による高速フィルタリング**: 各バケットは、自身が保持する 8 つのキーそれぞれのハッシュ値の上記 8 ビット（tophash）を、別の配列として保持している。キーを検索する際、tophash 配列をスキャンして検索対象キーと等しい tophash が存在するか確認する。tophash は 1 バイトであり、キー自体を探すよりもコストが下がり、不一致のキーを高速に除外できる。

### オーバーフロー処理

8 スロットが満杯のバケットに新しいキーが追加された場合、新しく「オーバーフローバケット」が確保される。元 t のバケットからこのオーバーフローバケットへのポインタが設定され、連結リストのようなチェーンが形成される。
元のバケットを検索して tophash が一致しなかった場合、このオーバーフローチェーンを辿って探索が続けられる。

### 負荷率と段階的リサイズ

map の性能を維持するために、バケット配列のサイズを適切に拡張(リサイズ)する必要がある。

- **リサイズのトリガー**: Go の map は 二つの条件でトリガーされる。
  1. バケットあたりの平均要素数が約 6.5（80%強）を超えた場合
  2. オーバーフローバケットの数が多すぎる場合（多すぎるとチェーンが長くなり、検索性能が低下する）
- **段階的リサイズ（または退避処理）**: Go に実装されているリサイズの仕組み。リサイズという高コストな処理の負荷が多数の操作に分散され、単一の操作による最悪実行時間が抑制される。
  - 多くのハッシュテーブル実装では、リサイズ時に全要素を一度に新しい大きな配列にコピーする方式（stop-the-world 方式）をとる
  - しかしこれだと、大規模な map では大きいレイテンシを引き起こす可能性がある
  - Go はサーバーサイドのアプリケーションのように低レイテンシが求められる用途で利用されることを想定されているため、この問題を回避する仕組みが組み込まれている
    - さらにいうと、Go はスループットよりも予測可能な低レイテンシを優先する設計思想があるらしい
  - リサイズがトリガーされると、Go は全要素をコピーするのではなく、現在のバケット配列を old_buckets として保持し、2 倍のサイズのバケット配列を確保する
  - その後の map への操作（挿入・更新・削除）の際、古いバケットから新しいバケットへデータを少しずつ（だいたい 1, 2 バケットずつ）移動する
    - 読み取りの際は、必要に応じて古いバケットと新しいバケットの両方を参照する

### 注意点

- **順序性なし**: 要素の反復順序は保証されない（Go 1.0 以降はランダム化されている）
- **並行安全性なし**: 複数の goroutine から同時アクセスする場合は`sync.Map`を使用する必要がある
- **メモリの保持**: Go の map は要素を削除しても確保したメモリを縮小することはないため、一時的に巨大になった map がメモリを保持し続ける可能性がある。
