...
Union-Find 木（または Disjoint Set Union, DSU、素集合データ構造）は、**複数の要素をグループ（集合）に分けて管理し、グループの統合（Union）や所属判定（Find）を高速に行うためのデータ構造**である。主にグラフの連結成分判定やクラスター管理、ネットワークの連結性判定などで利用される。

**このデータ構造の最大の利点は、集合の合併と所属判定をほぼ定数時間（アッカーマン関数の逆数オーダー）で実現できる。**
